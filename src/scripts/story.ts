  ...
  import("gsap").then(async ({ gsap }) => {
    ...
    media.add("(max-width: 1023px), (hover: none), (pointer: coarse)", () => {
      const effects = scenes.map((scene) => {
        const image = scene.querySelector<HTMLElement>("[data-scene-media]");
        if (!image) return null;
        return gsap.fromTo(image, { scale: 1.1, yPercent: -2 }, {
          scale: 1,
          yPercent: 2,
          ease: "none",
          scrollTrigger: { trigger: scene, start: "top bottom", end: "bottom top", scrub: 0.35 },
        });
      });
